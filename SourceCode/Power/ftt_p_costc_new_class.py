...
def cost_curves(BCET, BCSC, MEWD, MEWG, MEWL, MEPD, MERC, MRCL, RERY, MPTR, MRED, MRES, rti, t2ti, erti, year, dt, MERCX):
    ...
    tech_to_resource = [0, 1, 2, 2, 2, 2, 3, 3, 4, 4, 5, 6, 7, 8, 9, 10, 11, 11, 12, 13, 3, 3]
    ...
    MEPD[:, 5, 0] = (MEWG[:, 10, 0] \
                   + MEWG[:, 11, 0] * divide(BCET[:, 11, 13], BCET[:, 10, 13])) \
                    * 3.6/1000
    ...
    # Tidal
    MEPD[:, 7, 0] = MEWG[:, 12, 0] * 3.6/1000
    RERY[:, 7, 0] = copy.deepcopy(MEPD[:, 7, 0]) # in PJ
    # Hydro
    MEPD[:, 8, 0] = MEWG[:, 13, 0] * 3.6/1000
    RERY[:, 8, 0] = copy.deepcopy(MEPD[:, 8, 0]) # in PJ
    # Onshore
    MEPD[:, 9, 0] = MEWG[:, 14, 0] * 3.6/1000
    RERY[:, 9, 0] = copy.deepcopy(MEPD[:, 9, 0]) # in PJ
    # Offshore
    MEPD[:, 10, 0] = MEWG[:, 15, 0] * 3.6/1000
    RERY[:, 10, 0] = copy.deepcopy(MEPD[:, 10, 0]) # in PJ
    # Solar (PV + CSP)
    MEPD[:, 11, 0] = (divide(MEWG[:, 16, 0], BCET[:, 16, 13])  +  divide(MEWG[:, 17, 0], BCET[:, 17, 13])) * 3.6/1000
    RERY[:, 11, 0] = copy.deepcopy(MEPD[:, 11, 0]) # in PJ
    # Geothermal
    MEPD[:, 12, 0] = MEWG[:, 18, 0] * 3.6/1000
    RERY[:, 12, 0] = copy.deepcopy(MEPD[:, 12, 0]) # in PJ
    # Wave
    MEPD[:, 13, 0] = MEWG[:, 19, 0] * 3.6/1000
