...
def rldc(data, time_lag, iter_lag, year, titles):
    ...
    Sw = np.divide(np.sum(data['MEWG'][:, [onsh_ind, offsh_ind, wave_ind], 0], axis=1),
                   e_dem[:],
                   where=~np.isclose(e_dem[:], 0.0))
    Ss = np.divide(data['MEWG'][:, solar_ind, 0],
                   e_dem[:],
                   where=~np.isclose(e_dem[:], 0.0))
    ...
    vre_gr_sol = np.maximum((divide(data['MEWS'][:, solar_ind, 0] * data['MEWL'][:, solar_ind, 0],
                               time_lag['MEWS'][:, solar_ind, 0] * time_lag['MEWL'][:, solar_ind, 0])
                               -1.0)*data['MEWS'][:, solar_ind, 0],
                                0.005)
    
    vre_ggr_sol = 0.002 * data['MEWG'][:, solar_ind, 0]
    vre_ggr_sol = np.maximum((divide(data['MEWS'][:, solar_ind, 0] * data['MEWL'][:, solar_ind, 0],
                                 time_lag['MEWS'][:, solar_ind, 0] * time_lag['MEWL'][:, solar_ind, 0])
                                 -1.0)*data['MEWG'][:, solar_ind, 0],
                                 0.002 * data['MEWG'][:, solar_ind, 0])
    
    vre_gr_wind = 0.005 * np.ones(len(titles['RTI']))
    vre_gr_wind = np.maximum((divide(np.sum(data['MEWS'][:, wind_inds,  0] * data['MEWL'][:, wind_inds,  0], axis=1),
                                 np.sum(time_lag['MEWS'][:, wind_inds,  0] * time_lag['MEWL'][:, wind_inds,  0], axis=1))
                                 -1.0)*np.sum(data['MEWS'][:, wind_inds, 0], axis=1),
                                 0.005)
    
    vre_ggr_wind = 0.002 * np.sum(data['MEWG'][:, wind_inds,  0], axis=1)
    vre_ggr_wind = np.maximum((divide(np.sum(data['MEWS'][:, wind_inds, 0] * data['MEWL'][:, wind_inds,  0], axis=1),
                                 np.sum(time_lag['MEWS'][:, wind_inds, 0] * time_lag['MEWL'][:, wind_inds,  0], axis=1))
                                 -1.0)*np.sum(data['MEWG'][:, wind_inds, 0], axis=1),
                                 0.002 * np.sum(data['MEWG'][:, wind_inds, 0], axis=1))
    ...
    for r in range(len(titles['RTI'])):
        ...
        data['MCTG'][r, :, 0] = 0.0
        data['MCTG'][r, onsh_ind, 0] = curt_w[r]
        data['MCTG'][r, offsh_ind, 0] = curt_w[r]
        data['MCTG'][r, wave_ind, 0] = curt_w[r]
        data['MCTG'][r, solar_ind, 0] = curt_s[r]
...
#            if abs(mlsc_split_solar) > 0.0 and abs(data['MLSC'][r,0,0] - mlsc_split_wind) > 0.0:
            ...
            data['MLSP'][r, :, 0] = 0.0
            data['MLSP'][r, onsh_ind, 0] = data['MLSR'][r,0,0] * LSw
            data['MLSP'][r, offsh_ind, 0] = data['MLSR'][r,0,0] * LSw
            data['MLSP'][r, wave_ind, 0] = data['MLSR'][r,0,0] * LSw
            data['MLSP'][r, solar_ind, 0] = data['MLSR'][r,0,0] * LSs
        # For option 4, all VRE pay the equal amount
        elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
            data['MLSP'][r, :,0] = 0.0
            data['MLSP'][r, onsh_ind, 0] = data['MLSR'][r,0,0]
            data['MLSP'][r, offsh_ind, 0] = data['MLSR'][r,0,0]
            data['MLSP'][r, wave_ind, 0] = data['MLSR'][r,0,0]
            data['MLSP'][r, solar_ind, 0] = data['MLSR'][r,0,0] 
        ...
        if np.rint(data['MSAL'][r, 0, 0]) == 5:
            ...
            # Assign price values
            data['MSSP'][r,:,0] = 0.0
            data['MSSP'][r, onsh_ind, 0] = data['MSSR'][r,0,0] * SSw
            data['MSSP'][r, offsh_ind, 0] = data['MSSR'][r,0,0] * SSw
            data['MSSP'][r, wave_ind, 0] = data['MSSR'][r,0,0] * SSw
            data['MSSP'][r, solar_ind, 0] = data['MSSR'][r,0,0] * SSs
        # For option 4, all VRE pay the equal amount
        elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
            data['MSSP'][r,:,0] = 0.0
            data['MSSP'][r, onsh_ind, 0] = data['MSSR'][r,0,0]
            data['MSSP'][r, offsh_ind, 0] = data['MSSR'][r,0,0]
            data['MSSP'][r, wave_ind, 0] = data['MSSR'][r,0,0]
            data['MSSP'][r, solar_ind, 0] = data['MSSR'][r,0,0] 
        # All technologies pay the pay amount for the other options
        else:
        ...
        if np.rint(data['MSAL'][r, 0, 0]) in [3, 4, 5]:
            ...
            vre_long = vre + data['MEWG'][r, csp_ind, 0]
            ...
            if np.rint(data['MSAL'][r, 0, 0]) in [3]:
                marg_cost_sol_ls = total_output_solar_l * data['MLCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_sol[r])  - data['MLSP'][r, solar_ind, 0]
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_wind[r])  - data['MLSP'][r, onsh_ind, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
                marg_cost_sol_ls = total_output_solar_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_sol[r]) - data['MLSP'][r, solar_ind, 0]
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) - data['MLSP'][r, onsh_ind, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [5]:
                marg_cost_sol_ls = total_output_solar_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_sol[r]) * LSs - data['MLSP'][r, solar_ind, 0]
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) * LSw  - data['MLSP'][r, onsh_ind, 0] 
            
            data['MLSM'][r,:,0] = 0.0
            data['MLSM'][r,onsh_ind, 0] = marg_cost_wind_ls
            data['MLSM'][r,offsh_ind, 0] = marg_cost_wind_ls
            data['MLSM'][r,wave_ind, 0] = marg_cost_wind_ls
            data['MLSM'][r,solar_ind, 0] = marg_cost_sol_ls            
            ...
            if np.rint(data['MSAL'][r, 0, 0]) in [3]:
                marg_cost_sol_ss = output_sol * data['MSCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_sol[r])  - data['MSSP'][r, solar_ind, 0]
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_wind[r])  - data['MSSP'][r, onsh_ind, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
                marg_cost_sol_ss = output_sol * data['MSCC'][r,0,0] / (vre_long + vre_ggr_sol[r]) - data['MSSP'][r, solar_ind, 0]
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) - data['MSSP'][r, onsh_ind, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [5]:
                marg_cost_sol_ss = output_sol * data['MSCC'][r,0,0] / (vre_long + vre_ggr_sol[r]) * LSs - data['MSSP'][r, solar_ind, 0] 
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) * LSw  - data['MSSP'][r, onsh_ind, 0] 
            
            data['MSSM'][r,:,0] = 0.0
            data['MSSM'][r,onsh_ind, 0] = marg_cost_wind_ss
            data['MSSM'][r,offsh_ind, 0] = marg_cost_wind_ss
            data['MSSM'][r,wave_ind, 0] = marg_cost_wind_ss
            data['MSSM'][r,solar_ind, 0] = marg_cost_sol_ss 
